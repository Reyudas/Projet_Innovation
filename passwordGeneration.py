...
def train(input_line_tensor, target_line_tensor):
    ...
    for i in range(input_line_tensor.size(0)):
        ...
        # topv, topi = target_line_tensor[i].topk(1)
        # topi = topi[0][0]
        
        expected_char = all_letters[target_line_tensor[i].item()] if target_line_tensor[i].item() < len(all_letters) else "00000"
        
        outv, outi = output.topk(1)
        outi = outi[0][0]
        
        output_char = all_letters[outi] if outi < len(all_letters) else "00001"
        correct += (output_char == expected_char)
        #print("output:",output_char,"expected:",expected_char)

    loss.backward()

    accuracy = correct/input_line_tensor.size(0)
...
def training(n_epochs, lines):
    ...
    for iter in range(1, n_epochs + 1):
        output, loss, accuracy = train(*randomTrainingExample(lines))
        total_loss += loss
        total_accuracy += accuracy

        if iter % print_every == 0:
            print('%s (%d %d%%) %.4f (%.4f) %s' % (timeSince(start), iter, iter / n_iters * 100, total_loss / iter, loss, total_accuracy/iter))
    ...
    if nb_samples > 0:
        ...
        for i in range(1, nb_samples + 1):
            ...
            while predicted in predicted_current:
                starting_letters = ""
                for n in range(nc):
                    rc = random.randint(0, len(string.ascii_letters+string.digits) - 1)
                    starting_letters = starting_letters + (string.ascii_letters+string.digits)[rc]
        ...
        while accuracy < p:
            ...
            if predicted in lineTest:
                accuracy = accuracy + 1
                print(starting_letters, ' -> ', predicted, ' accuracy: ', accuracy, ' founded over ', l, ' names in ', timeSinceStart(start), 'and', i, 'epochs => coverage of', percent,'% of total names names =', p,'\n')
...
if __name__ == '__main__':
    ...
    #
    parser.add_argument("-d", "--trainingData", default="data/Password/Ashley-Madison.txt", type=str,
    ...
    lines = getLines(repData)
    train_set, test_set = split(args.s, lines)

    print('filenameTrain: ', repData)
